...
| Ingestion & Processing | High-volume batch loops across 150+ universe records or file trees. | Deep-dive focus on individual companies, filings, transcripts, or plans. |
...
3. Rule 3: The Hybrid Pipeline Pattern
   ...
   - Phase 2 (Deterministic Calculation & Verification): The agent feeds these inputs into a deterministic script (`valuation_model.py`, `return_engine.py`, `validate_thesis.py`) to calculate mathematical outputs and verify schema compliance.

4. Rule 4: Data Auditing & Errata Identification
   When checking whether all 150 universe stocks meet a standard or finding missing fields, always run or write a deterministic Python/Node audit script. Never browse 150 individual files manually in an LLM context loop.

## 5. Summary Anti-Patterns to Avoid

- Anti-Pattern 1 (Agent Hallucinating Batch Audits): An agent claiming "I verified all 150 stocks and found no errors" without running an audit script.
