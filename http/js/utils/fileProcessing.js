// utils/fileProcessing.js - Native CSV parsing (no PapaParse dependency)
...
/**
 * Native CSV parser that handles quoted fields, escaped quotes, and commas
 * @param {string} csvLine - A single line of CSV data
 ...
 */
export const parseCSVLine = (csvLine, delimiter = ',', quoteChar = '"') => {
  const fields = [];
  let currentField = '';
  ...
  try {
    ...
    for (let i = 0; i < Math.min(10, lines.length); i++) {
      ...
      if (possibleHeaderLine.includes('"Symbol"') || possibleHeaderLine.includes('Symbol')) {
        ...
        // Parse the header line with native CSV parser
        const parsed = parseCSVLine(possibleHeaderLine, ',', '"');
        
        if (parsed && parsed.length > 0) {
          columnHeaders = parsed.map(header => header.trim());
        }
        break;
      }
    }
