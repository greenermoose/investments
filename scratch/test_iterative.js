...
function runFifoForSymbol(sym) {
  ...
  let preInceptionInitialQty = 0;
  let totalSplitRatio = 1;
  let splitRatios = new Map();
  let splitFactorsAfter = new Array(symTx.length).fill(1);

  // Iterate to converge on pre-inception quantity and split ratios
  for (let iter = 0; iter < 5; iter++) {
    let runQty = preInceptionInitialQty;
    splitRatios.clear();
    ...
    for (let i = 0; i < symTx.length; i++) {
      ...
      if (action.includes('buy') || action.includes('reinvest') || action.includes('assigned')) {
        ...
        // Round to nearest standard split ratio (e.g. 10, 3, 20, 1.5, 0.0667, etc.)
        // This is important because the running quantity may be slightly off due to rounding or missing small dividend reinvestments.
    ...
    totalSplitRatio = currentFactor;

    // Recalculate finalTxQty
    let finalTxQty = 0;
    for (let i = 0; i < symTx.length; i++) {
      const tx = symTx[i];
      const action = tx.action.toLowerCase();
    ...
    const newPreInceptionInitialQty = preInceptionFinalQty / totalSplitRatio;
    
    // Check for convergence
    if (Math.abs(newPreInceptionInitialQty - preInceptionInitialQty) < 0.0001) {
      preInceptionInitialQty = newPreInceptionInitialQty;
      break;
    }
    preInceptionInitialQty = newPreInceptionInitialQty;
  }
  ...
  return {
    ...
    splitRatios: Array.from(splitRatios.values())
  };
}
