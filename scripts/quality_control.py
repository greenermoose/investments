...
class QualityController:
    ...
    def fix_all(self, target_symbol=None, log_errata=True):
        ...
        # 2. Regenerate Master Universe Catalog via full-fidelity builder (ERR-2026-08-014 fix)
        print("Rebuilding master universe.json via build_universe_json.py (full-fidelity path)...")
        build_script = os.path.join(os.path.dirname(__file__), "build_universe_json.py")
        subprocess.run([sys.executable, build_script], cwd=os.path.dirname(os.path.dirname(__file__)), check=False)
        self.universe = self._load_json(self.universe_path, default=[])
        fixed_count += 1
